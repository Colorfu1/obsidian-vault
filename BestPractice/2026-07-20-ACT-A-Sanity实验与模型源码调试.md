# 2026-07-20 ACT A-Sanity 实验与模型源码调试

## 重要进展

- 完成五条 LIBERO demonstration 的 ACT A-Sanity 启动器设计与脚本，固定关键训练配置并
  默认关闭外部实验上传。
- 明确 teacher-forced offline prediction 与 LIBERO closed-loop rollout 的评测边界、输出和
  指标，避免用离线误差代替任务成功率。
- 追踪 LeRobot 数据统计生成方式，确认 state/action 统计与图像抽样统计的来源和用途。
- 梳理 Accelerate 与 BF16 autocast 的 device、精度、backward 和梯度裁剪职责。
- 从源码层面分析 ACT 的 VAE、attention、ResNet backbone 和二维位置编码数据流。
- 当天尚未启动正式 20k-step A-Sanity 训练，因此没有 checkpoint 或正式评测结果。
...
[2026-07-16-LeRobot-ACT调试与ALOHA数据可视化.md](2026-07-16-LeRobot-ACT调试与ALOHA数据可视化.md)
的训练入口、VS Code Debug 和 ALOHA
可视化工作。
